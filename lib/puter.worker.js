const PROJECT_PREFIX = "rommify_project_";

const jsonError = (status, message, extra = {}) => {
  return new Response(JSON.stringify({ status, message, ...extra }, null, 2), {
    status,
...
router.post("/api/projects/save", async ({ request, user }) => {
  try {
    ...
    if (!project?.id || !project?.sourceImage)
      return jsonError(400, "Project and source image are required");
